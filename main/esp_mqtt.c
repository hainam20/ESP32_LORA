...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_DATA:
        ...
        uint8_t buf[256];
        if (strcmp((char *)event->data, "0") == 0)
        {
            int send_len = sprintf((char *)buf, "ON_ID%d", 0);
            lora_send_packet(buf, send_len);
            vTaskDelay(pdMS_TO_TICKS(1000));
            break;
        }
        else if (strcmp((char *)event->data, "1") == 0)
        {
            int send_len = sprintf((char *)buf, "OFF_ID%d", 0);
            lora_send_packet(buf, send_len);
            vTaskDelay(pdMS_TO_TICKS(1000));
            break;
        }
        else if (strcmp((char *)event->data, "2") == 0)
        {
            int send_len = sprintf((char *)buf, "ON_ID%d", 1);
            lora_send_packet(buf, send_len);
            vTaskDelay(pdMS_TO_TICKS(1000));
            break;
        }
        else if (strcmp((char *)event->data, "3") == 0)
        {
            int send_len = sprintf((char *)buf, "OFF_ID%d", 1);
            lora_send_packet(buf, send_len);
            vTaskDelay(pdMS_TO_TICKS(1000));
            break;
        }
        else if (strcmp((char *)event->data, "4") == 0)
        {
            int send_len = sprintf((char *)buf, "ON_ID%d", 2);
            lora_send_packet(buf, send_len);
            vTaskDelay(pdMS_TO_TICKS(1000));
            break;
        }
        else if (strcmp((char *)event->data, "5") == 0)
        {
            int send_len = sprintf((char *)buf, "OFF_ID%d", 2);
            lora_send_packet(buf, send_len);
            vTaskDelay(pdMS_TO_TICKS(1000));
            break;
        }
